<#
.SYNOPSIS
  Admin Setup. Desktop icon, Grok Bot + Chrome offer, wipe leftover apps, hide tray, restart.
#>
[CmdletBinding()]
param(
  [switch]$Apply,
...
if ($self -and (Test-Path -LiteralPath $self)) {
  try {
    ...
    if ((Resolve-Path $self).Path -ne $persistResolved) { Copy-Item -LiteralPath $self -Destination $persist -Force }
  } catch { Copy-Item -LiteralPath $self -Destination $persist -Force -ErrorAction SilentlyContinue }
}
if (-not (Test-Path -LiteralPath $persist) -and $self -and (Test-Path $self)) { Copy-Item -LiteralPath $self -Destination $persist -Force }
...
function New-AdminSetupShortcut([string]$LnkPath) {
  ...
  $sc.WindowStyle = 1
  $sc.Description = 'Clear apps, hide tray, restart'
...
function Confirm-ResetIfAppsMissing {
  $g = Test-GrokBotInstalled; $c = Test-ChromeInstalled
  if ($g -and $c) { return $true }
  $missing = @(); if (-not $g) { $missing += 'Grok Bot' }; if (-not $c) { $missing += 'Google Chrome' }
  $lines = @('These apps are still not installed:','', (($missing | ForEach-Object { "- $_" }) -join "`r`n"),'','Yes = continue wipe / tray / restart.','No  = stop.')
  $r = [Windows.Forms.MessageBox]::Show(($lines -join "`r`n"), 'Admin Setup - confirm reset', [Windows.Forms.MessageBoxButtons]::YesNo, [Windows.Forms.MessageBoxIcon]::Warning)
...
function Invoke-ClearApps {
  ...
  $protect = @('Realtek*','Microsoft Visual C++*','Microsoft Visual Studio* Redistributable*','Microsoft .NET*','Microsoft Edge*','Microsoft Edge WebView2*','Microsoft Update*','Windows PC Health Check*','Update for *','Security Update*','Hotfix*','Intel*','NVIDIA*','AMD*','Chipset*','Canon *','Printer*','Driver*','Google Chrome*','Grok Bot*','Windows Terminal*','Windows SDK*','Microsoft OneDrive*')
  ...
  foreach ($prog in @(Get-ItemProperty $paths -ErrorAction SilentlyContinue)) {
    ...
    if (-not $u) { continue }
    Write-Host "Uninstall $dn"; Write-Log "Uninstall $dn"
  ...
  try {
    foreach ($pkg in @(Get-AppxPackage -AllUsers -ErrorAction SilentlyContinue)) {
      $n = [string]$pkg.Name; if (-not $n) { continue }
      ...
      try { Remove-AppxPackage -Package $pkg.PackageFullName -AllUsers -ErrorAction SilentlyContinue; $changed = $true } catch {
...
if ($doWipe -or $doRestart) {
  if (-not (Confirm-ResetIfAppsMissing)) { Write-Host "Reset skipped. Log: $log"; return }
}
...
function Set-MinimalTaskbar {
  ...
  try {
    ...
    if ($langs -and $langs.Count -gt 0) { Set-WinUserLanguageList -LanguageList $langs[0] -Force -ErrorAction SilentlyContinue }
  } catch {}
  try {
    $pinDir = Join-Path $env:APPDATA 'Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar'
    if (Test-Path -LiteralPath $pinDir) { Get-ChildItem -LiteralPath $pinDir -Force -ErrorAction SilentlyContinue | Remove-Item -Force -ErrorAction SilentlyContinue }
  ...
  try { Get-ChildItem 'HKCU:\Control Panel\NotifyIconSettings' -ErrorAction SilentlyContinue | ForEach-Object { Set-RegDword $_.PSPath 'IsPromoted' 0 } } catch {}
}
function Invoke-RemoveLeftoverInstalledList {
  Write-Host 'Removing leftover Installed apps rows (Snipping Tool, winget source)...'
  Write-Log 'Leftover Installed apps'
  try {
    ...
    Get-AppxProvisionedPackage -Online | Where-Object { $_.DisplayName -match 'ScreenSketch|Snipping' } | ForEach-Object { Remove-AppxProvisionedPackage -Online -PackageName $_.PackageName -ErrorAction SilentlyContinue }
  } catch {}
  try {
    if (Get-Command winget -ErrorAction SilentlyContinue) {
      winget source remove --name winget 2>$null | Out-Null
      winget uninstall --name "Windows Package Manager Source (winget) V2" --accept-source-agreements --disable-interactivity --silent 2>$null | Out-Null
      winget uninstall --id Microsoft.Winget.Source --accept-source-agreements --disable-interactivity --silent 2>$null | Out-Null
    }
    ...
    Get-AppxProvisionedPackage -Online | Where-Object { $_.DisplayName -match 'Winget.Source|Package Manager Source' } | ForEach-Object { Remove-AppxProvisionedPackage -Online -PackageName $_.PackageName -ErrorAction SilentlyContinue }
  } catch {}
}
if ($doWipe) {
  Invoke-ClearApps | Out-Null
  Invoke-RemoveLeftoverInstalledList
...
if ($doRestart) {
  ...
  shutdown.exe /r /t 60 /c 'Admin-Setup finished. Apps cleared. Tray hidden.'
}
