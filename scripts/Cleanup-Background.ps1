...
function Get-SettingsStyleInstalledAppCount {
  <#
    Rough count matching Settings > Apps > Installed apps:
    Win32 uninstall registry entries + current-user AppX that look like user-visible apps.
  #>
  ...
  foreach ($pkg in $pkgs) {
    ...
    # Exclude Frameworks / Native / Structural / Microsoft Windows.* framework-style packages
    ...
    if ($name -like 'Microsoft.Windows.*' -and $name -notmatch '(?i)(Calculator|Notepad|Photos|Camera|Alarms|SoundRecorder|Terminal|Paint|Clock|Maps|MediaPlayer)') {
      # keep known consumer Windows apps; skip other Microsoft.Windows.* platform packages
      if ($name -match '(?i)(ShellExperience|StartMenuExperience|CloudExperience|CBS|BioEnrollment|Print|DevicesFlow|ParentalControls|PeopleExperience|LockApp|Search|ImmersiveControlPanel|FileManager|Narrator|InputApp|CaptivePortal|CallingShellApp|AssignedAccess|SecureAssessment|AppResolver|PinningConfirmationDialog|XGpuEjectDialog)') {
        continue
      }
      # Most Microsoft.Windows.* without consumer app names are platform — skip unless DisplayName-like known apps
      if ($name -notmatch '(?i)(Calculator|Notepad|Photos|Camera|Alarms|SoundRecorder|Terminal|Paint|StickyNotes|Todos|YourPhone|DevHome)') {
        continue
      }
    }
...
if (($Apply -or $UninstallNotKept -or $UninstallJunk) -and -not (Test-IsAdmin)) {
  ...
  $argList += @('-KeepProfile', $KeepProfile)
...
[pscustomobject]@{
  SettingsStyleTotal = $settingsApps.SettingsStyleTotal
  Win32Count         = $settingsApps.Win32Count
  AppxVisibleCount   = $settingsApps.AppxVisibleCount
...
$appxJunk = @(
  ...
  'Microsoft.Windows.Photos','Microsoft.WindowsCamera','Microsoft.WindowsCalculator',
  'Microsoft.WindowsAlarms','Microsoft.WindowsSoundRecorder','Microsoft.MicrosoftStickyNotes',
  'Microsoft.Paint','Microsoft.ScreenSketch','Microsoft.Todos','Microsoft.PowerAutomateDesktop',
  'Microsoft.OutlookForWindows','Microsoft.GamingApp','MicrosoftCorporationII.QuickAssist',
  'Clipchamp.Clipchamp','MicrosoftWindows.Client.WebExperience','Microsoft.WidgetsPlatformRuntime',
  'Microsoft.MicrosoftOfficeHub','Microsoft.OneConnect','Microsoft.BingSearch'
)
if ($KeepProfile -eq 'DriversOnly') {
  # Broader Store-app purge for DriversOnly (still skip frameworks via Remove-AppxPackage failures)
  $appxJunk += @(
    'Microsoft.WindowsStore','Microsoft.StorePurchaseApp','Microsoft.DesktopAppInstaller',
    'Microsoft.WindowsFeedbackHub','Microsoft.Getstarted','Microsoft.WindowsMaps'
  )
}
...
foreach ($rk in @(
  ...
  foreach ($n in @($props.PSObject.Properties | Where-Object { $_.Name -notin @('PSPath','PSParentPath','PSChildName','PSDrive','PSProvider') })) {
    ...
    # Bare startup: do NOT specially keep Chrome/Grok (SecurityHealth already protected above)
    if (-not $isJunk) {
...
foreach ($dir in @([Environment]::GetFolderPath('Startup'), "$env:ProgramData\Microsoft\Windows\Start Menu\Programs\Startup")) {
  ...
  Get-ChildItem $dir -Force -ErrorAction SilentlyContinue | ForEach-Object {
    ...
    # Bare startup: clear all shortcuts including Chrome/Grok (uninstall keep list is separate)
...
# --- Unpin + uninstall Edge / Outlook / Store ---
...
if (($Apply -or $UninstallNotKept) -and (Test-Path $unpinHelper)) {
  ...
  if ($Apply) {
    ...
    try {
      ...
      $results.Add((Add-Result 'Explorer restart' 'FIXED' 'taskbar refresh'))
      $script:ChangesMade = $true
    } catch {
      $results.Add((Add-Result 'Explorer restart' 'NEED' $_.Exception.Message))
    }
  }
}
